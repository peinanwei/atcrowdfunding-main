...
<form class="form-inline" role="form" style="float:left;">
  ...
  <button type="button" class="btn btn-warning" onclick="queryAdv()"><i class="glyphicon glyphicon-search"></i> 查询</button>
</form>
<button type="button" class="btn btn-danger" onclick="deleteAdvs()" style="float:right;margin-left:10px;"><i class=" glyphicon glyphicon-remove"></i> 删除</button>
<button type="button" class="btn btn-primary" style="float:right;" onclick="window.location.href='${APP_PATH}/manager/advAdd.htm'"><i class="glyphicon glyphicon-plus"></i> 新增</button>
<br>
 <hr style="clear:both;">
	  ...
	  <script type="text/javascript" src="${APP_PATH}/script/menu.js"></script>
        <script type="text/javascript">
            ...
            function pageQuery(pageIndex) {
            	...
            	$.ajax({
            		url : "${APP_PATH}/manager/advPageQuery.do",
            		type : "POST",
            		data : obj,
            		beforeSend : function() {
            		...
            		success : function( result ) {
            			...
            			if ( result.success ) {
            				...
            				var advList = pageObj.datas;
            				var content = "";
            				$.each(advList, function(i, adv){
            					// 循环体
             				   ...
             				   content = content +  '<tr>';
          					   ...
          					   content = content +  '  <td><input type="checkbox" value="'+adv.id+'"></td>';
          	                   content = content +  '  <td>'+adv.name+'</td>';
          					   ...
          					   content = content +  '      <button type="button" onclick="window.location.href=\'${APP_PATH}/manager/advEdit.htm?pageNo='+pageObj.pageNo+'&id='+adv.id+'\'" class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';
          					   content = content +  '	  <button type="button"  onclick="deleteAdv('+adv.id+',\''+adv.name+'\')" class="btn btn-danger btn-xs"><i class=" glyphicon glyphicon-remove"></i></button>';
          					   content = content +  '  </td>';
          	                   content = content +  '</tr>';
            				});
            ...
            function deleteAdv(id,name){
            	layer.confirm("刪除广告为"+name+"的用户嘛？",  {icon: 3, title:'提示'}, 
			            	function(cindex){
            					var loadingIndex = -1;
            					$.ajax({
            						url:"${APP_PATH}/manager/deleteAdv.do",
            						type:"POST",
            						data:{
            							id : id
            						},
            						beforeSend : function() {
            	            			loadingIndex = layer.msg('广告删除中', {icon: 16});
            	            			return true;
            	            		},
            	            		success : function( result ) {
            	            			layer.close(loadingIndex);
            	            			if ( result.success ) {
            	            				layer.msg("广告删除成功", {time:1000, icon:6, shift:6});
            	            				pageQuery(1);
            	            			}else{
            	            				layer.msg("广告删除失败", {time:1000, icon:6, shift:6});
            	            				pageQuery(1);
            	            			}
            	            		}
            ...
            function deleteAdvs(){
            	...
            	if ( checkBox.length == 0 ) {
            		...
            		layer.confirm("删除选择的广告数据, 是否继续？",  {icon: 3, title:'提示'}, function(cindex){
        				...
        				$.ajax({
        					url : "${APP_PATH}/manager/deleteAdvs.do",
        					type : "POST",
        					data  : obj,
        					success : function(result) {
        						if ( result.success ) {
