	...
	<div class="container theme-showcase" role="main">
		...
		<form role="form" style="margin-top: 20px;">
			<div class="form-group">
				...
				<input id="authcode" type="text" class="form-control" placeholder="请输入你邮箱中接收到的验证码">
			</div>
			<button type="button" onclick="javascript:;" class="btn btn-primary">重新发送验证码</button>
			<button id="checkemail" type="button" class="btn btn-success">申请完成</button>
		</form>
		<hr>
	</div>
	<!-- /container -->
		...
		$("#checkemail").click(function(){
			...
			$.ajax({
				...
				success:function(result){
					if(result.success){
						layer.msg("验证成功", {time:1000, icon:5, shift:6});
						window.location.href="${APP_PATH}/member/member.htm";
					}else{
						layer.msg("输入的验证码有误,请重新输入", {time:1000, icon:5, shift:6});
					}
				}
			})
		});
